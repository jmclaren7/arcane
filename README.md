...
>
> The full, up-to-date list of changes that set this fork apart from upstream — and the notes for re-applying them when rebasing — lives in **[FORK.md](FORK.md)**. In short, the fork carries Docker build-context and dev-container fixes, expanded contributor docs, CI workflows adapted to run on a fork without upstream-only infrastructure, and a set of GitOps/UI refinements (honest manual-sync feedback with a per-row spinner, faster shallow git clones, abbreviated commit hashes, opt-in injection of the synced commit into the project's env, a way to convert a Git-synced project back into a regular project, a copy button that hides itself when the Clipboard API is unavailable, and quieter startup logs that stop reporting non-problems), an include-mode switch that inverts the auto-update and auto-heal container exclusion lists into allowlists, and a deploy fix that falls back to building a service from its `build` section when its `image` can't be pulled, matching `docker compose up`. It also carries a temporary startup repair for databases that an early `next` image left with a mis-numbered migration.
>
